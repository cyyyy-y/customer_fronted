  ...
  <el-card shadow="none">
    <el-table
      ...
      <el-table-column
          ...
          label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.rebornState === null"
                     type="info" size="small" round @click="reborn(scope.row)">重生
          </el-button>
          <el-button v-if="scope.row.rebornState === 0"
                     type="info" size="small" round plain @click="rebornDes(scope.row)">审核中
          </el-button>
          <el-button v-if="scope.row.rebornState === 2"
                     type="danger" size="small" round plain disabled>已驳回
          </el-button>
          <el-button v-if="scope.row.reviewState === 1 && scope.row.rebornState === 1"
                     type="success" size="small" round plain disabled>已重生
          </el-button>
        </template>
      </el-table-column>
      <el-table-column
...
export default {
  ...
  methods: {
    ...
    reborn(row) {
      this.$emit('reborn', row)
    },
    rebornDes(row) {
      this.$emit('rebornDes', row)
    }
  }
}
</script>
